...
const ROUTINES = [
  { id: 1, name: 'Manejo de Balón', duration: '30 min', level: 'Principiante', icon: 'basketball', reps: 'Técnica' },
  { id: 2, name: 'Salto Vertical', duration: '35 min', level: 'Intermedio', icon: 'human-handsup', reps: '6 × 8' },
  { id: 3, name: 'Explosividad', duration: '40 min', level: 'Intermedio', icon: 'run-fast', reps: '5 series' },
  { id: 4, name: 'Defensa & Agilidad', duration: '45 min', level: 'Avanzado', icon: 'shield-outline', reps: '4 × 10' },
  { id: 5, name: 'Condición Física NBA', duration: '60 min', level: 'Avanzado', icon: 'trophy-outline', reps: '5 vueltas' },
];
...
export default function BasquetScreen() {
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <LinearGradient colors={['#dc6120', '#9a3412']} style={styles.hero} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
          <TouchableOpacity style={styles.backBtn} onPress={() => router.back()}>
            <MaterialCommunityIcons name="arrow-left" size={22} color="#ffffff" />
          </TouchableOpacity>
          <Text style={styles.heroEmoji}>🏀</Text>
          <Text style={styles.heroTitle}>Básquet</Text>
          <Text style={styles.heroSub}>Agilidad & Explosividad</Text>
          <View style={styles.heroBadges}>
            <View style={styles.heroBadge}><Text style={styles.heroBadgeText}>12 Rutinas</Text></View>
            <View style={styles.heroBadge}><Text style={styles.heroBadgeText}>Recreativo a Competitivo</Text></View>
          </View>
        </LinearGradient>
        ...
        <View style={styles.body}>
          ...
          {ROUTINES.map((r) => (
            <TouchableOpacity key={r.id} style={styles.routineCard} activeOpacity={0.8}>
              <View style={[styles.routineIcon, { backgroundColor: '#fff7ed' }]}>
                <MaterialCommunityIcons name={r.icon as any} size={26} color="#dc6120" />
              </View>
