...
const ROUTINES = [
  { id: 1, name: 'Carrera de Resistencia', duration: '30 min', level: 'Principiante', icon: 'run', reps: '5 km' },
  { id: 2, name: 'Intervalos HIIT', duration: '25 min', level: 'Intermedio', icon: 'run-fast', reps: '8 series' },
  { id: 3, name: 'Fartlek Progresivo', duration: '40 min', level: 'Intermedio', icon: 'timer-outline', reps: '10 km' },
  { id: 4, name: 'Tempo Run', duration: '35 min', level: 'Avanzado', icon: 'speedometer', reps: '8 km' },
  { id: 5, name: 'Long Run Weekend', duration: '90 min', level: 'Avanzado', icon: 'map-marker-path', reps: '20 km' },
];
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <LinearGradient colors={['#f97316', '#c2410c']} style={styles.hero} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
          <TouchableOpacity style={styles.backBtn} onPress={() => router.back()}>
            <MaterialCommunityIcons name="arrow-left" size={22} color="#ffffff" />
          </TouchableOpacity>
          <Text style={styles.heroEmoji}>🏃</Text>
          <Text style={styles.heroTitle}>Running</Text>
          <Text style={styles.heroSub}>Cardio & Resistencia</Text>
          <View style={styles.heroBadges}>
            <View style={styles.heroBadge}><Text style={styles.heroBadgeText}>18 Rutinas</Text></View>
            <View style={styles.heroBadge}><Text style={styles.heroBadgeText}>Principiante a Avanzado</Text></View>
          </View>
        </LinearGradient>
          ...
          {ROUTINES.map((r) => (
            <TouchableOpacity key={r.id} style={styles.routineCard} activeOpacity={0.8}>
              <View style={[styles.routineIcon, { backgroundColor: '#fff7ed' }]}>
                <MaterialCommunityIcons name={r.icon as any} size={26} color="#f97316" />
              </View>
